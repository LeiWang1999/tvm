...
@T.prim_func
def func(A: T.Buffer[(1024, 1024, 16, 16), "float16"], B: T.Buffer[(1024, 1024, 16, 16), "float16"], C: T.Buffer[(1024, 1024, 16, 16), "float32"]):
    # function attr dict
    ...
    C_warp = T.alloc_buffer([1024, 1024, 32, 8], dtype="float32", scope="warp")
    for ii_0 in T.thread_binding(128, thread="blockIdx.y"):
        for jj_0 in T.thread_binding(128, thread="blockIdx.x"):
            for ii_1 in T.thread_binding(2, thread="threadIdx.y"):
                for jj_1 in T.thread_binding(2, thread="threadIdx.z"):
                    for ii_2_init in T.serial(4, annotations={"thread_rasterization":10}):
                        for jj_2_init in T.serial(4):
                            with T.block("B_init_o"):
                                vii = T.axis.spatial(1024, ii_0 * 8 + ii_1 * 4 + ii_2_init)
                                vjj = T.axis.spatial(1024, jj_0 * 8 + jj_1 * 4 + jj_2_init)
                                vi_o = T.axis.spatial(1, 0)
                                vj_o = T.axis.spatial(1, 0)
                                T.reads()
                                T.writes(C_warp[vii, vjj, 0 : 32, 0 : 8])
                                C_warp_1 = T.match_buffer(C_warp[vii, vjj, 0 : 32, 0 : 8], [32, 8], dtype="float32", scope="warp", offset_factor=1)
                                T.launch_thread(tx, 32)
                                T.mma_fill(8, C_warp_1.data, C_warp_1.elem_offset, dtype="float32")
                    for kk_0 in T.serial(512, annotations={"software_pipeline_order":[0, 1, 2], "software_pipeline_stage":[0, 0, 1]}):
                                    ...
                                    for ax0_ax1_ax2_ax3_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                                        for ax0_ax1_ax2_ax3_fused_4 in T.vectorized(8):
                                            with T.block("B_shared"):
                                                v0 = T.axis.spatial(1024, jj_0 * 8 + (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 1024 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) // 512)
                                                v1 = T.axis.spatial(1024, kk_0 * 2 + (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 1024 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) % 512 // 256)
                                                v2 = T.axis.spatial(16, (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 1024 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) % 256 // 16)
                                                v3 = T.axis.spatial(16, (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 1024 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) % 16)
                        ...
                        for kk_1 in T.serial(2):
                            ...
                            for ax0 in T.serial(4):
                                with T.block("B_shared_warp_o"):
                                    v0 = T.axis.spatial(1024, jj_0 * 8 + jj_1 * 4 + ax0)
                                    v1 = T.axis.spatial(1024, kk_0 * 2 + kk_1)
                                    v2_o = T.axis.spatial(1, 0)
                                    v3_o = T.axis.spatial(1, 0)
                            ...
                            for ii_2, jj_2 in T.grid(4, 4):
                                with T.block("B_update_o"):
                                    ...
                                    vjj = T.axis.spatial(1024, jj_0 * 8 + jj_1 * 4 + jj_2)
                                    ...
                                    C_1 = T.match_buffer(C_warp[vii, vjj, 0 : 32, 0 : 8], [32, 8], dtype="float32", scope="warp", offset_factor=16)
                                    T.launch_thread(tx, 32)
                                    T.ptx_mma("m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_1.data, A_1.elem_offset + tx * 8, B_1.data, B_1.elem_offset + tx * 8, C_1.data, C_1.elem_offset + tx * 8, False, dtype="float32")
                                    T.ptx_mma("m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_1.data, A_1.elem_offset + tx * 8, B_1.data, B_1.elem_offset + tx * 8 + T.FloorDiv(8, 2), C_1.data, C_1.elem_offset + tx * 8 + T.FloorDiv(8, 2), False, dtype="float32")
                    for ax0, ax1 in T.grid(4, 4):
                        with T.block("C_warp_o"):
                            v0 = T.axis.spatial(1024, ii_0 * 8 + ii_1 * 4 + ax0)
                            v1 = T.axis.spatial(1024, jj_0 * 8 + jj_1 * 4 + ax1)
                            v2_o = T.axis.spatial(1, 0)
                            v3_o = T.axis.spatial(1, 0)
                            T.reads(C_warp[v0, v1, 0 : 32, 0 : 8])
                            T.writes(C[v0, v1, 0 : 16, 0 : 16])
                            C_warp_2 = T.match_buffer(C_warp[v0, v1, 0 : 32, 0 : 8], [32, 8], dtype="float32", scope="warp", offset_factor=1)
                            C_2 = T.match_buffer(C[v0, v1, 0 : 16, 0 : 16], [16, 16], dtype="float32", strides=[C_s0, C_s1], offset_factor=1)
                            T.launch_thread(tx, 32)
                            T.mma_store(16, 16, T.tvm_access_ptr(T.type_annotation(dtype="float32"), C_2.data, C_2.elem_offset, C_s0 * 16, 2, dtype="handle"), C_warp_2.data, C_warp_2.elem_offset, C_s0, dtype="float32")
