...
@main = primfn(a: handle, b: handle, c: handle) -> ()
  ...
  buffers = {A: Buffer(A_1: Pointer(global float16), float16, [1024, 1024, 16, 16], []),
             ...
             C: Buffer(C_1: Pointer(global float32), float32, [1024, 1024, 16, 16], [])}
  ...
  block([], "root") {
    ...
    C_warp = alloc_buffer(float32[1024, 1024, 32, 8])
    for (ii_0: int32, 0, 128) "thread_binding" {
      for (jj_0: int32, 0, 128) "thread_binding" {
            ...
            for (ii_2_init: int32, 0, 4) {
              for (jj_2_init: int32, 0, 4) {
                block([1024, 1024, 1, 1], "B_init_o") as [vii, vjj, vi_o, vj_o] {
                  ...
                  bind(vjj, (((jj_0*8) + (jj_1*4)) + jj_2_init))
                  bind(vi_o, 0)
                  bind(vj_o, 0)
                  tir.reads([])
                  ...
                  @tir.mma_fill(8, C_warp_2: Pointer(warp float32), elem_offset: int32, dtype=float32)
              }
            }
                    ...
                    for (ax0_ax1_ax2_ax3_fused_3_1: int32, 0, 32) "thread_binding" {
                      for (ax0_ax1_ax2_ax3_fused_4_1: int32, 0, 8) "vectorized" {
                        block([1024, 1024, 16, 16], "B_shared") as [v0_1, v1_1, v2_1, v3_1] {
                          bind(v0_1, ((jj_0*8) + floordiv((((((ax0_ax1_ax2_ax3_fused_0_1*2048) + (ax0_ax1_ax2_ax3_fused_1_1*1024)) + (ax0_ax1_ax2_ax3_fused_2_1*256)) + (ax0_ax1_ax2_ax3_fused_3_1*8)) + ax0_ax1_ax2_ax3_fused_4_1), 512)))
                  ...
                  for (jj_2: int32, 0, 4) {
                    block([1024, 1024, tir.reduce_axis(0, 1024), 1, 1, tir.reduce_axis(0, 1)], "B_update_o") as [vii_1, vjj_1, vkk, vi_o_1, vj_o_1, vk_o] {
                      ...
                      attr [IterVar(tx_3: int32, [0:32], "ThreadIndex", "threadIdx.x")] "thread_extent" = 32 {
                        @tir.ptx_mma("m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_3: Pointer(warp float16), (elem_offset_5: int32 + (tx_3*8)), B_3: Pointer(warp float16), (elem_offset_6: int32 + (tx_3*8)), C_3: Pointer(warp float32), (elem_offset_7: int32 + (tx_3*8)), False, dtype=float32)
                        @tir.ptx_mma("m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_3, (elem_offset_5 + (tx_3*8)), B_3, ((elem_offset_6 + (tx_3*8)) + floordiv(8, 2)), C_3, ((elem_offset_7 + (tx_3*8)) + floordiv(8, 2)), False, dtype=float32)
                      }
                  }
                }
              }
            ...
            for (ax0_2: int32, 0, 4) {
              for (ax1: int32, 0, 4) {
                block([1024, 1024, 1, 1], "C_warp_o") as [v0_4, v1_4, v2_o_2, v3_o_2] {
                  ...
                  @tir.mma_store(16, 16, @tir.tvm_access_ptr(@tir.type_annotation(, dtype=float32), C_5: Pointer(global float32), elem_offset_8: int32, (C_s0: int32*16), 2, dtype=handle), C_warp_4: Pointer(warp float32), elem_offset_9: int32, C_s0, dtype=float32)
              }
            }
          }
        }
