...
def flashattn(batch, heads, seq_len, dim, is_casual, block_M, block_N):
    ...
    score_shape = [T.int32(batch), T.int32(heads), T.int32(seq_len), T.int32(seq_len)]  # Shape for similarity scores
    dtype = "float16"
    accum_dtype = "float"
    ...
    def main(
        ...
        Similarity_Scores: T.Buffer(score_shape, dtype),
    ):
        with T.Kernel(T.ceildiv(T.int32(seq_len), T.int32(block_M)), T.int32(heads), T.int32(batch), threads=T.int32(128)) as (bx, by, bz):
            ...
            T.copy(Q[T.int32(bz), T.int32(bx) * T.int32(block_M) : (T.int32(bx) + T.int32(1)) * T.int32(block_M), T.int32(by), T.int32(0):T.int32(dim)], Q_shared)
            T.fill(acc_o, 0)
            T.fill(logsum, 0)
            ...
            for k in T.Pipelined(T.int32(loop_range), num_stages=1):
                ...
                if is_casual:
                    for i, j in T.Parallel(T.int32(block_M), T.int32(block_N)):
                        acc_s[i, j] = T.if_then_else(
                            bx * T.int32(block_M) + i >= k * T.int32(block_N) + j, 0, -T.infinity(acc_s.dtype)
                        )
                else:
                    T.clear(acc_s)
                ...
                T.copy(acc_s_cast, Similarity_Scores[bz, by, bx * block_M : (bx + 1) * block_M, k * block_N : (k + 1) * block_N])
